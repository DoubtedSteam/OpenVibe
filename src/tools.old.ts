...
// Re-export for backward compatibility (imported by ToolExecutor et al.)
export { workspaceFileExistsRelative };
...
// (getWorkspaceRoot, resolveWorkspacePath, workspaceFileExistsRelative, readLines,
//  writeLines, splitLinesForEditInput, splitLinesNormalized, inferCrlfForNewFile
//  are now imported from './utils/pathHelpers')
 ...
 * Uses VS Code's workspace.findFiles API for file discovery, then reads and
 * matches line-by-line. Returns up to maxResults matches (default 50, max 200).
 */
export async function grepSearchTool(params: GrepSearchParams): Promise<string> {
  try {
    ...
    // Find candidate files via VS Code's fast file-search

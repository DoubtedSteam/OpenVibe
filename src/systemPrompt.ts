...
- **edit** — Edit a line range. To insert without removing lines, set endLine = startLine - 1 with non-empty newContent. Pass empty newContent "" to delete lines — ⚠️ empty newContent with endLine >= startLine = IRREVERSIBLE DELETION (no undo); only pass empty when you truly intend to delete. **Required before each edit on existing files:** read_file or find_in_file on that path (host-enforced). A secondary LLM check verifies the change. For multiline content, pass the full text directly in newContent — JSON string escaping handles newlines and quotes.
